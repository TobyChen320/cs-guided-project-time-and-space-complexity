...
#     # if we get through the nested loops we can then return "No two sum found"
...
def two_sum(nums, target):  # O(n)
    ...
    for i in range(len(nums)): # O(n)
        ...
        # and a value of nums at the index i
    ...
    for i in range(len(nums)): # O(n)
        ...
        # and the value of dictionary at the key of compliment is not equal to i
        if compliment in my_table and my_table[compliment] != i: # O(1)
            # return a list [i, the value of dictionary at the key of compliment]
